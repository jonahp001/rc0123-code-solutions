...
export function useTodos() {
  ...
  function toggleCompleted(todoId) {
    ...
    todoComplete.isCompleted = !todoComplete.isCompleted

    async function check() {
      const newTodo = await updateTodo(todoComplete)
      const todosArray = todos.map(entry => {
        return entry.todoId === newTodo.todoId ? newTodo : entry
      })
      setTodos(todosArray)
    }
    check()
